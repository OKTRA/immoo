...
import { syncOverduePaymentsForAgency } from "@/services/payment/paymentAutoSync";
...
export default function AgencyPaymentsPage() {
  ...
  const { data: payments = [], isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        // 1) Synchroniser automatiquement les paiements en retard/manquants pour l'agence
        const syncRes = await syncOverduePaymentsForAgency(agencyId);
        if (syncRes.error) {
          console.warn('Sync des paiements en retard (agence) a échoué:', syncRes.error);
        } else if (syncRes.totalSynced > 0) {
          toast.success('Paiements synchronisés', {
            description: `${syncRes.totalSynced} ligne(s) de paiement ont été ajoutées automatiquement.`
          });
        }

        // 2) Récupérer les propriétés de l'agence
        ...
        // 3) Récupérer les baux pour ces propriétés
        const { data: leases, error: leasesError } = await supabase
          .from('leases')
          .select('id, property_id, tenant_id, start_date')
          .in('property_id', propertyIds);
        if (leasesError) throw leasesError;
        const leaseIds = (leases || []).map(l => l.id);
        if (leaseIds.length === 0) return [];

        // 4) Récupérer les locataires pour hydrater le nom
        const tenantIds = Array.from(new Set((leases || []).map(l => l.tenant_id).filter(Boolean)));
        let tenantsById = new Map<string, { first_name?: string; last_name?: string; email?: string }>();
        ...
        // 5) Récupérer les paiements par lease_id
        const { data: paymentsData, error: paymentsError } = await supabase
          .from('payments')
          .select('id, lease_id, amount, payment_date, due_date, payment_method, status, transaction_id, notes, payment_type, is_auto_generated, processed_by, created_at')
          .in('lease_id', leaseIds)
          .order('created_at', { ascending: false });
        if (paymentsError) throw paymentsError;

        // Maps utilitaires
        const propsById = new Map<string, { title?: string }>();
        (props || []).forEach(p => propsById.set(p.id, p));
        const leasesById = new Map<string, { property_id?: string; tenant_id?: string; start_date?: string }>();
        (leases || []).forEach(l => leasesById.set(l.id, l));

        // 6) Transformer les données
        const transformedPayments: PaymentWithDetails[] = (paymentsData || []).map(payment => {
          const lease = leasesById.get(payment.lease_id);
          const property = lease?.property_id ? propsById.get(lease.property_id) : undefined;
          const tenant = lease?.tenant_id ? tenantsById.get(lease.tenant_id) : undefined;
          return {
            id: payment.id,
            ...
            propertyTitle: property?.title || t('agencyDashboard.pages.payments.unknownProperty'),
            tenantName: tenant ? `${tenant.first_name || ''} ${tenant.last_name || ''}`.trim() : t('agencyDashboard.pages.payments.unknownTenant'),
